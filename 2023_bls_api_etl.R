...
## DO IT FOR STATES (BETTER!)
## CREATE GITHUB PROJECT AND SET UP GITHUB ACTION (https://rfortherestofus.com/2023/05/github-actions/)
### HANDLE PACKAGES
### THINK ABOUT HOW TO HANDLE API KEYS IN GITHUB 
## STREAMLINE CODE SO 
...
## set_bls_key("e75d562cec584b7bb3b847a50de75d5f", overwrite = TRUE)
## 35655cc2988a448a87837cf13ec57533
...
us_total_states<-blscrapeR::area_titles %>% 
  filter(str_detect(area_title,"U.S. TOTAL") |str_detect(area_title,"-- Statewide"))
...
ces_plots<-function(data=df_1_clean, start_date="2019-12-01"){
  ...
  plot_1<-data %>% 
    ...
    scale_color_manual(values=set_1_no_yellow)+
    labs(title = "Recent evolution of Nofarm employment",
         color="Sector",
...
plots_cities_1<-ces_plots(data=df_cities_1_clean)

plots_cities_1[[2]]

plots_1<-ces_plots(data=df_1_clean)
plots_1[[2]]
